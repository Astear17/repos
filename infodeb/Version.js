function compatible(works_min, works_max, tweak_compatibility) {
   ...
   if (currentiOS < works_min) {
      el.innerHTML = "Phiên bản iOS của bạn quá thấp đối với Tweaks này. Tweak  này hoạt động trên " + tweak_compatibility + " 📵";
      el.classList.add("677b86")
   } else if (currentiOS > works_max) {
      el.innerHTML = "Phiên bản iOS của bạn quá mới đối với Tweaks này. Tweaks này hoạt động trên " + tweak_compatibility + " 😑";
      el.classList.add("677b86")
   } else if (String(currentiOS) != "NaN") {
      el.innerHTML = "Tweaks này hoạt động trên thiết bị của bạn 😍";
